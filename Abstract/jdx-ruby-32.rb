...
class JdxRuby32 < Formula
 ...
 def install
    ...
    # Fix file permissions: Ruby's make install sets files to 0444 (read-only),
    # which prevents gem install from upgrading bundled gems.
    # Make all installed files user-writable to match compile-from-source behavior (0644).
    # See: https://github.com/jdx/mise/discussions/7268
    ohai "Fixing file permissions in #{prefix}"
    system "chmod", "-R", "u+w", prefix.to_s
  ...
  def test
    ...
    # Test that gem files are user-writable (not 0444 from make install)
    # See: https://github.com/jdx/mise/discussions/7268
    gemspec = Dir["#{testpath}/lib/ruby/gems/*/specifications/*.gemspec"].first
    assert gemspec, "Expected at least one gemspec file"
    assert File.writable?(gemspec), "Gemspec #{gemspec} should be user-writable"
